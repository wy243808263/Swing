...
public class MemberFrame extends JFrame implements MouseListener {

	private static final long serialVersionUID = 9121212372019239918L;
	...
	// 表格面板表格头部
	String[] title = { "主键", "会员编号", "姓     名", "性     别", "年     龄", "身份证号", "联系方式", "地     址", "入会时间", "完美卡号" };
	// 表格面板表格模型
	...
	public void init() {
		...
		lblDate = new JLabel("\u52A0\u5165\u65E5\u671F");
		lblDate.setForeground(Color.WHITE);
		lblDate.setBounds(739, 408, 54, 15);
		getContentPane().add(lblDate);

		lblCard = new JLabel("\u5B8C\u7F8E\u5361\u53F7");
		lblCard.setForeground(Color.WHITE);
		lblCard.setBounds(739, 470, 54, 15);
		...
		txtUserID.setBounds(800, 467, 120, 25);
		...
		txtIdCard.setForeground(Color.WHITE);
		txtIdCard.setColumns(10);
		txtIdCard.setBounds(800, 258, 120, 25);
		...
		femaleRadioBtn.setBounds(860, 165, 45, 23);
		...
		updateBtn.setBounds(284, 571, 125, 56);
		...
		delBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				int rows = table.getSelectedRow();// 得到某一行的行数
				if (rows > 0) {
					int uid = Integer.valueOf(table.getValueAt(rows, 0).toString());

					Object[] options = { "确定", "取消" };
					int bool = JOptionPane.showOptionDialog(table, "确认要删除选中的信息", "温馨提示", JOptionPane.YES_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
					if (bool == 0) {
						if (memberDao.del(uid)) {
							JOptionPane.showMessageDialog(addBtn, "删除成功");
							...
							JOptionPane.showMessageDialog(addBtn, "删除失败", "温馨提示", JOptionPane.WARNING_MESSAGE);
						}
					}
				}
			}
		...
		updateBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String userId = txtUserID.getText();// 完美卡号
				String card = txtIdCard.getText();// 身份证
				int memNo = Integer.valueOf(table.getValueAt(table.getSelectedRow(), 0).toString());

				member.setAddress(addr);
				member.setId(memNo);
				member.setJoinDate(date);
				member.setTelePhone(phoneNo);
				member.setUserID(userId);
				member.setAge(Integer.valueOf(age));
				member.setAddress(addr);
				member.setUserName(name);
				member.setUserNo(empNo);
				member.setUserCard(card);

				boolean bool = memberDao.update(member);
				if (bool) {
		...
		addBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				Member member = getAllTxt(0);
				boolean bool = memberDao.save(member);
				if (bool) {
					JOptionPane.showMessageDialog(addBtn, "添加成功");
					tableModel.fireTableDataChanged();
					fullEmp();
				} else {
	 ...
	 * @return
	 */
	public Member getAllTxt(int bool) {
		Member member = new Member();
		if (maleRadioBtn.isSelected()) {
			member.setSex("男");
		} else {
	...
	private void fullEmp() {
		List<Member> memList = memberDao.searchAll();
		tableModel = new DefaultTableModel(null, title);// 设置表格头部以及显示信息
		for (int i = 0; i < memList.size(); i++) {
			Member member = (Member) memList.get(i);
			Object[] rowData = { member.getId(), member.getUserNo(), member.getUserName(), member.getSex(), member.getAge(), member.getUserCard(), member.getTelePhone(), member.getAddress(),
					member.getJoinDate(), member.getUserID() };
			tableModel.addRow(rowData);
		}
		table.setModel(tableModel); // 设置表格
		// 隐藏表格中的：身份证号、联系方式、联系地址、入职时间列
		Tools.HiddenCell(table, 0);
		Tools.HiddenCell(table, 5);
		Tools.HiddenCell(table, 6);
		Tools.HiddenCell(table, 7);
		Tools.HiddenCell(table, 8);
	}
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (row > 0) {
			...
			txtAddr.setText(table.getValueAt(row, 7).toString());
			txtDate.setText(table.getValueAt(row, 8).toString());
			txtUserID.setText(table.getValueAt(row, 9).toString());// 完美卡号
			txtIdCard.setText(table.getValueAt(row, 5).toString());// 身份证

			if ("男".equals(table.getValueAt(row, 3).toString())) {
